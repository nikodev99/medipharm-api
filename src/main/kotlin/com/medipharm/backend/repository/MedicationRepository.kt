    ...
    @Query("""
        ...
        WHERE m.is_active = true
          AND (
            ...
            OR similarity(m.name_lowercase, :query) > 0.3
            OR similarity(m.dci_lowercase, :query) > 0.3
          )
        ORDER BY
          CASE
            WHEN m.name_lowercase ILIKE :query THEN 1             -- correspondance exacte insensible à la casse
            WHEN m.dci_lowercase ILIKE :query THEN 2
            WHEN m.name_lowercase ILIKE '%' || :query || '%' THEN 3
            ELSE 4
          END,
        ...
        LIMIT 50
    """)
    fun searchMedication(query: String): Flux<Medication>
